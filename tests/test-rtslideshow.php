...
class Test_rtslideshow extends WP_UnitTestCase {
	...
	public function test_form_submission() {
		...
		// Set up test data.
		$image_ids = array( 1, 10, 20 );
		$form_data = array(
			'rt_slideshow_image_ids' => implode( ',', $image_ids ),
		);

		// Set up form submission.
		$this->set_post_data( $form_data );
		$this->set_permalink_structure( '/%postname%/' );

		// Mock WP media library response with selected image IDs.
		$media_response = array(
			'success' => true,
			'selected' => $image_ids,
		);
		add_filter( 'ajax_query_attachments_args', function( $query ) use ( $image_ids ) {
			$query['post__in'] = $image_ids;
			return $query;
		} );
		add_filter( 'wp_prepare_attachment_for_js', function( $response ) use ( $media_response ) {
			if ( in_array( $response['id'], $media_response['selected'] ) ) {
				$response['url'] = 'http://domain.com/' . $response['id'] . '.jpg';
			}
			return $response;
		} );

		// Simulate clicking "Add Image" button to open media library.
		$this->click( '#rt_slideshow_add_image_button' );

		// Simulate selecting images in media library.
		foreach ( $image_ids as $id ) {
			$this->click( "#attachment-$id .check" );
		}
		$this->click( '.media-modal-close' );

		// Call the form handler function.
		do_action( 'admin_post_rt_slideshow_settings_save' );

		// Check that the image IDs were saved in the database.
		$saved_image_ids = get_option( 'rt_slideshow_image_ids' );
		$this->assertEquals( $form_data['rt_slideshow_image_ids'], $saved_image_ids );
